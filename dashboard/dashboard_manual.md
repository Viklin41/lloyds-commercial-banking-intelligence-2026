...
```
cd C:\Users\visha\Lloyds_Github\lloyds-commercial-banking-intelligence-2026
```

**Step 2.** Run the server. The file to run is **`dashboard/serve.py`**:

```
C:/Python314/python.exe dashboard/serve.py
```

On this machine, use **Python 3.14** specifically. It is the interpreter that has DuckDB, Flask and
pandas installed. The other Pythons on the machine do not.

**Step 3.** It opens `http://127.0.0.1:8000` in your browser by itself after about a second.

**What you should see in the terminal.** If you see this, it worked:

```
data    : C:\Users\visha\Lloyds_Github\data
...
| `--data D:\somewhere` | Point at a different copy of the data tree. |

---

## 4. Where the data lives

This matters, because **the data is not in the repository**. The files are far too large to commit.

- **The code** is in the git repo: `lloyds-commercial-banking-intelligence-2026\dashboard\`
- **The data** is outside it: `C:\Users\visha\Lloyds_Github\data\`
...
3. Otherwise the built-in default path above.

If it cannot find the files it prints the paths it tried and stops, rather than starting up empty.
...
| `shortlist_reasons_2026-07.parquet` | SHAP reasons. **This one lives in the repo**, in `dashboard/post_build_change/` |
...
| `ModuleNotFoundError: duckdb` | Wrong Python | Use `C:/Python314/python.exe` |
| "Could not find the company parquet" | Data tree moved | Pass `--data` with the right folder |
