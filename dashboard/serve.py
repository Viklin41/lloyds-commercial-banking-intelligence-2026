...
# The data tree is not committed: it holds files far too large for git. It is
# resolved in three steps: --data on the command line, then the LLOYDS_DATA
# environment variable, then `data/` beside this repository, which is where
# scripts/fetch_dashboard_data.py unpacks the release bundle. The default is
# repo-relative rather than machine-specific, so a fresh clone that has run the
# fetch script needs no flags and no edits to this file.
...
def main():
    ...
    ap.add_argument("--data", default=None, metavar="DIR",
                    help="root of the data tree holding processed/, market_analysis/ "
                         "and processed/sam_sc/. Also settable with the LLOYDS_DATA "
    ...
    if not Path(PARQUET).exists():
        raise SystemExit(
            ...
            "Run `python scripts/fetch_dashboard_data.py` to download and unpack\n"
            "the data bundle, or point at an existing copy with --data DIR or by\n"
            "setting LLOYDS_DATA. That directory needs processed/ (with\n"
            "sam_sc/data/ inside it) and market_analysis/.\n")
    print(f"data    : {DATA}")
