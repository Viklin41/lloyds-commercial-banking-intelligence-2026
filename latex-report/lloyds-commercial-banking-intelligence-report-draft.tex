...
We compared the two outcomes to determine if this was the right. Restricted to rows
where both labels are defined, a company that proposes a strike-off is less likely than
average to enter genuine insolvency, at a lift of 0.59. The two also separate cleanly and in
opposite directions on company age: voluntary exit runs at 14.6\% for companies under two years
old and falls to 3.1\% past twenty, while genuine insolvency runs at 0.05\% under two years and
rises with age. Those are two different phenomena, a dormant shell being abandoned and an established trading business failing, and merging them would have produced a risk model
...
One further definitional care: the \texttt{segment} field mixes real size tiers with filing states such as Dormant and No Filings, so a naive "segment changed" test runs at about thirteen per cent over
twelve months and is mostly filing churn rather than growth. The growth label therefore ranks
only the four genuine tiers, and companies with no rank are excluded from the label rather than
assigned to one.
