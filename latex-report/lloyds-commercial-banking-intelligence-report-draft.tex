...
\note{The literature. Altman's Z-score and the ratio-based tradition, and why it
does not transfer: it assumes audited financial statements, and the companies we
care about file abbreviated accounts or none. Then the behavioural or
filing-based alternative (late filings, dormancy, status changes) which is what
actually works at this size. Also cover demand-side prediction, which is much
thinner in the literature than distress prediction, and say so, because it is
part of why lending readiness is the more novel of our four targets. Peer
reviewed sources, plus one or two from the FT or the Economist on SME credit
supply for context.}
...
\section{How the project organised itself, and 3 times reorganised}
\label{sec:orgs}
\note{Short, one page at most, but do not skip it, because it is the evidence of
project management the template asks for and it is genuinely informative.
Split 1 mirrored the client's three teams (Growth, Attrition, Existing
Relationships) and broke within two weeks, because the three needs are three
questions asked of one dataset, not three datasets. Split 2 was by data type,
structured against unstructured, and it held until the unstructured half
measured its own coverage and found there was nothing there, at which point the
team re-formed again around the structured core plus a delivery layer. Say
plainly that both reorganisations were driven by measurement. Footnote the
dating honestly: no commit records either decision, and the dates are inferred
from when branches stopped and from what people started next. Table of branches
and owners; the full record is Appendix~\ref{appx:chronology}.}
...
\section{Structured Feature engineering: CH + any additional source}
